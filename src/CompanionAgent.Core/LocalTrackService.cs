...
public sealed class LocalTrackService : ILocalTrackService
{
    ...
    public LocalTrackService(string? acPath = null)
    {
        var acPathOverride = string.IsNullOrWhiteSpace(acPath) ? null : acPath;
        _data = new(() => Load(acPathOverride), LazyThreadSafetyMode.ExecutionAndPublication);
    }
    ...
    private static (List<TrackDto>, Dictionary<string, string>) Load(string? acPathOverride)
    {
        var tracks = new List<TrackDto>();
        var outlinePaths = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        var basePath = FindAcTracksPath(acPathOverride);
    ...
    private static string? FindAcTracksPath(string? acPathOverride = null)
    {
        if (!string.IsNullOrWhiteSpace(acPathOverride))
        {
            var overridePath = Path.Combine(acPathOverride, "content", "tracks");
