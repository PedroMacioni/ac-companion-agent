...
public sealed class SyncWorker : IDisposable
{
    ...
    public SyncWorker(SupabaseClient supabase, ILocalHistoryService history, ILocalTrackService trackService, LogService? log = null)
    {
        _supabase = supabase;
        _history = history;
    ...
    private void LogActivity(string message, LogLevel level = LogLevel.Info, LogCategory category = LogCategory.Sync)
    {
        ActivityLogged?.Invoke(message);
        _log?.Log(level, category, message);
    }
    ...
    public async Task SyncAsync()
    {
        ...
        {
            LogActivity("Sincronização já em andamento...");
            return;
        }
        try
        {
            ...
            LogActivity("Iniciando sincronização...");
            ...
            if (newSessions.Count > 0)
            {
                ...
                foreach (var session in newSessions)
                {
                    try
                    {
                        ...
                        if (laps.Count > 0)
                        {
                            ...
                            LogActivity($"  Sessão {shortId}: {laps.Count} voltas", LogLevel.Success);
                        }
                        else
                        {
                            LogActivity($"  Sessão {shortId}: 0 voltas — retry no próximo ciclo", LogLevel.Warning);
                        }
                    }
                    catch (Exception ex)
                    {
                        var shortId = session.Id[..Math.Min(8, session.Id.Length)];
                        LogActivity($"  Sessão {shortId}: erro ao ler voltas — {ex.Message}", LogLevel.Error);
                        ...
                        LogActivity($"  Sessão {shortId}: agendada para retry", LogLevel.Warning);
                    }
                }
                ...
                LogActivity($"{newSessions.Count} {(newSessions.Count == 1 ? "sessão nova" : "sessões novas")} sincronizadas ({totalNewLaps} voltas)", LogLevel.Success);
            }
            else
            {
                LogActivity("Sessões em dia — nenhuma novidade", LogLevel.Success);
            }
            ...
            {
                LogActivity($"Retentando voltas para {lapRetryIds.Count} {(lapRetryIds.Count == 1 ? "sessão" : "sessões")}...");
                ...
                foreach (var id in lapRetryIds)
                {
                    try
                    {
                        ...
                        if (laps.Count > 0)
                        {
                            await _supabase.UpsertLapsAsync(laps);
                            recoveredIds.Add(id);
                            LogActivity($"  Sessão {id[..Math.Min(8, id.Length)]}: {laps.Count} voltas recuperadas", LogLevel.Success);
                        }
                    }
                ...
                if (recoveredIds.Count > 0)
                {
                    ...
                    LogActivity($"  {recoveredIds.Count} sessões com voltas recuperadas", LogLevel.Success);
                }
            }
            ...
            LogActivity($"─────────────────────────────────────");
            LogActivity($"Sincronização concluída às {DateTime.Now:HH:mm:ss}", LogLevel.Success);
            ...
            LogActivity("─────────────────────────────────────");
            LogActivity("Agente pronto · abra o dashboard para ver seus dados", LogLevel.Success);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.Unauthorized)
        {
            _consecutiveFailures++;
            LogActivity($"Erro: {ex.Message}", LogLevel.Error);
            StateChanged?.Invoke(SyncState.Error, "Conta desconectada — abra as configurações");
        }
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.BadRequest)
        {
            _consecutiveFailures++;
            LogActivity($"Erro de dados: {ex.Message}", LogLevel.Error);
            StateChanged?.Invoke(SyncState.Error, "Erro nos dados de sincronização");
        }
        catch (Exception ex)
        {
            _consecutiveFailures++;
            LogActivity($"Erro: {ex.GetType().Name}: {ex.Message}", LogLevel.Error);
    ...
    private async Task<bool> SyncLapsAsync(string sessionId)
    {
        try
        {
            ...
            LogActivity($"  Retry sessão {shortId}: {laps.Count} voltas sincronizadas", LogLevel.Success);
            return true;
        }
        catch (Exception ex)
        {
            var shortId = sessionId[..Math.Min(8, sessionId.Length)];
            LogActivity($"  Retry sessão {shortId}: {ex.Message}", LogLevel.Error);
            return false;
        }
    }
    ...
    private async Task SyncTracksAsync(CancellationToken ct = default)
    {
        ...
        if (pendingOutlines.Count > 0)
            LogActivity($"Enviando outlines de pistas (0/{pendingOutlines.Count})...");
        ...
        for (int i = 0; i < pendingOutlines.Count; i++)
        {
            ...
            if ((i + 1) % 5 == 0 || i == pendingOutlines.Count - 1)
                LogActivity($"Enviando outlines de pistas ({i + 1}/{pendingOutlines.Count})...");
        }

        await _supabase.UpsertTracksAsync(allTracks, outlineUrls,
            (done, total) => LogActivity($"Pistas sincronizadas ({done}/{total})..."), ct);
        ...
        LogActivity($"{allTracks.Count} pistas catalogadas · {outlineUrls.Count} outlines enviados", LogLevel.Success, LogCategory.Tracks);
    }
    ...
    private async Task SyncCarBadgesAsync(CancellationToken ct = default)
    {
        ...
        LogActivity($"Enviando previews de carros (0/{allCarIds.Count})...", LogLevel.Info, LogCategory.Cars);
        ...
        for (int i = 0; i < allCarIds.Count; i++)
        {
            ...
            try
            {
                ...
                if (bytes.Length > 0 && await _supabase.UploadCarPreviewAsync(carId, bytes, contentType, linkedCts.Token))
                    ...
                    LogActivity($"  preview não enviado: {carId}", LogLevel.Warning, LogCategory.Cars);
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
            catch (Exception ex)
            {
                LogActivity($"  erro no preview {carId}: {ex.Message}", LogLevel.Error, LogCategory.Cars);
            }

            attempted.Add(carId);

            if ((i + 1) % 5 == 0 || i == allCarIds.Count - 1)
                LogActivity($"Enviando previews de carros ({i + 1}/{allCarIds.Count})...", LogLevel.Info, LogCategory.Cars);
        }

        if (attempted.Count > 0)
            _cache.MarkCarBadgesSynced(attempted);

        LogActivity(uploaded.Count > 0
            ? $"{uploaded.Count}/{attempted.Count} previews de carros enviados"
            : $"Previews de carros: nenhum novo para enviar", LogLevel.Success, LogCategory.Cars);
    }
    ...
    private async Task SyncCarSpecsAsync(CancellationToken ct = default)
    {
        ...
        LogActivity($"Lendo specs de carros (0/{carIds.Count})...", LogLevel.Info, LogCategory.Cars);
        ...
        for (int i = 0; i < carIds.Count; i++)
        {
            ...
            if ((i + 1) % 10 == 0 || i == carIds.Count - 1)
                LogActivity($"Lendo specs de carros ({i + 1}/{carIds.Count})...", LogLevel.Info, LogCategory.Cars);
        }
        ...
        if (specs.Count > 0)
        {
            ...
            LogActivity($"{specs.Count} specs de carros sincronizadas", LogLevel.Success, LogCategory.Cars);
        }
        else
        {
            LogActivity("Specs: nenhuma nova para sincronizar", LogLevel.Success, LogCategory.Cars);
        }
    }
    ...
    private async Task SyncCarSetupsAsync(CancellationToken ct = default)
    {
        ...
        if (setups.Count > 0)
        {
            ...
            LogActivity($"{setups.Count} setups sincronizados", LogLevel.Success, LogCategory.Cars);
        }
        else
        {
            LogActivity("Setups: nenhum encontrado", LogLevel.Success, LogCategory.Cars);
        }
    }
